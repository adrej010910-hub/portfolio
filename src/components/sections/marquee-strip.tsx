...
export function MarqueeStrip() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // scroll-driven x offset for depth (GSAP ScrollTrigger)
      gsap.fromTo(
        sectionRef.current,
        { opacity: 0.2 },
        {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative overflow-hidden border-y border-white/[0.06] bg-white/[0.02] py-5"
      aria-hidden
    >
      <div
      ...
      <div ref={trackRef} className="flex w-max items-center gap-10 whitespace-nowrap">
        {[...items, ...items].map((item, i) => (
          <span key={i} className="flex items-center gap-10">
            <span className="font-display text-lg font-semibold uppercase tracking-wider text-slate-300/80">
              {item}
            </span>
